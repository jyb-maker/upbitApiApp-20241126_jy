...
class UpbitApi(QThread):  # 시그널 클래스->스레드 클래스
    ...
    def run(self):
        while self.alive:  # 무한루프(3초에 한번씩 실행)
            ...
            trade_price = pyupbit.get_current_price(self.ticker)  # 입력된 코인 가격 가져오기
            print(trade_price)
            self.coinDataSent.emit(float(trade_price))  # 시그널 함수인 coinDataSenet 가져온 코인가격 데이터를 제출
...
class MainWindow(QMainWindow, form_class):  # 슬롯 클래스
    ...
    def comboBox_active(self):  # 콤보박스의 메뉴가 변경되었을 때 호출되는 메소드
        ...
        self.upbitapi.close()  # while문의 무한루프가 정지 -> 시그널 클래스 객체가 삭제
